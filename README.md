    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .stats-bar {
            ...
            padding: 12px 20px; border-radius: 10px; font-weight: 600; margin-top: 15px;
            font-size: clamp(12px, 3vw, 16px);
        }
        .stats-bar span { color: var(--accent); }

        #board {
            display: grid; gap: 6px; width: 100%; max-width: 450px; aspect-ratio: 1 / 1;
            margin: 20px auto; background: var(--panel); padding: 10px; border-radius: 12px;
            box-shadow: 0 10px 25px rgba(0,0,0,0.3);
        }

        .cell {
            background: var(--cell-empty); color: var(--cell-text); border-radius: 8px;
            display: flex; align-items: center; justify-content: center; cursor: pointer;
            font-weight: 700; font-size: clamp(16px, 4vw, 24px);
            transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1); user-select: none;
            touch-action: manipulation; box-shadow: inset 0 -3px 0 rgba(0,0,0,0.1);
        }

        .cell:active { transform: scale(0.92); box-shadow: inset 0 0 0 rgba(0,0,0,0.1); }
        .cell.valid { background: var(--accent); color: white; }
        .cell.current { 
            background: var(--win); color: white; box-shadow: 0 0 15px rgba(16, 185, 129, 0.6);
            transform: scale(1.05); z-index: 2;
        }
        .cell.possible { background: #e2e8f0; border: 3px dashed var(--accent); opacity: 0.8; }
        .cell.invalid { background: var(--lose) !important; color: white; animation: shake 0.3s ease-in-out; }
        ...
        .progress-bar { width: 100%; height: 12px; background: #334155; border-radius: 6px; overflow: hidden; box-shadow: inset 0 2px 4px rgba(0,0,0,0.2); }
        .progress-fill { height: 100%; width: 0%; background: linear-gradient(90deg, var(--accent), var(--win)); transition: width 0.3s ease; }

        /* Estilos do Ranking */
        .tabs { display: flex; justify-content: center; gap: 8px; margin-top: 10px; flex-wrap: wrap; }
        .tab { padding: 8px 16px; background: #334155; border-radius: 8px; cursor: pointer; font-size: 14px; font-weight: 600; transition: all 0.2s; }
        .tab:hover { background: #475569; }
        .tab.active { background: var(--accent); box-shadow: 0 4px 10px rgba(59, 130, 246, 0.4); }
        .rank-item { background: rgba(255,255,255,0.05); margin: 8px auto; padding: 12px; width: 100%; max-width: 400px; border-radius: 8px; display: flex; justify-content: space-between; align-items: center; font-size: 14px; border: 1px solid rgba(255,255,255,0.05); }
        .rank-item.win { border-left: 6px solid var(--win); }
        .rank-item.lose { border-left: 6px solid var(--lose); }
        .tag { font-size: 11px; padding: 3px 8px; border-radius: 12px; background: #0f172a; margin-left: 8px; font-weight: 600; text-transform: uppercase; }

        /* MODAL CUSTOMIZADO (VITORIA/DERROTA) */
        .modal-overlay {
        ...
        .modal-box {
            background: var(--panel); padding: 40px 30px; border-radius: 20px;
        ...
        .modal-box.modal-win { border-color: var(--win); box-shadow: 0 10px 40px rgba(16, 185, 129, 0.3); }
        .modal-box.modal-lose { border-color: var(--lose); box-shadow: 0 10px 40px rgba(239, 68, 68, 0.3); }

        .modal-icon { font-size: 50px; margin-bottom: 10px; line-height: 1; }
        .modal-title { font-size: 28px; font-weight: 700; margin-bottom: 5px; }
        .modal-win .modal-title { color: var(--win); }
        .modal-lose .modal-title { color: var(--lose); }
        
        .modal-reason { color: var(--text-muted); font-size: 15px; margin-bottom: 20px; }
        .modal-score { font-size: 36px; font-weight: 700; color: white; margin-bottom: 30px; }
        .modal-score span { font-size: 16px; color: var(--text-muted); display: block; font-weight: 400; }
        
        .modal-btn { width: 100%; padding: 15px; font-size: 16px; border-radius: 12px; }

        footer { margin-top: 40px; color: var(--text-muted); font-size: 12px; line-height: 1.5; padding-bottom: 20px;}
    </style>
</head>
...
<div class="panel">
    <details class="instructions-panel">
        ...
        <div class="instructions-content">
            ...
            <p><strong>Início:</strong> O primeiro clique define a sua posição inicial.</p>
    ...
    <div class="controls">
        <input type="text" id="playerName" value="Jogador" placeholder="O seu Nome">
        <select id="mode">
    ...
    <div class="controls">
        ...
        <button onclick="resetGame()" style="background: #475569;">Reiniciar Tabuleiro</button>
    </div>
...
<footer>
    Jogo do Cavalo v5.5 (Modal & Regras Finais)<br>
...
<div class="modal-overlay" id="gameOverModal">
    <div class="modal-box" id="modalBox">
        ...
        <div class="modal-title" id="modalTitle">VITÓRIA!</div>
        <div class="modal-reason" id="modalReason">Completou o tabuleiro!</div>
        <div class="modal-score">
            <span>Pontuação Final</span>
            <div id="modalScoreValue">1500</div>
        </div>
        <button class="modal-btn" onclick="closeModalAndRestart()">Jogar Novamente</button>
    </div>
</div>
...
<script>
    ...
    function playSound(type) {
        ...
        if (type === 'click') {
            ...
            [400, 600, 800, 1000].forEach((freq, i) => oscillator.frequency.setValueAtTime(freq, now + (i*0.1)));
            gainNode.gain.setValueAtTime(0.3, now);
    ...
    function resetGame() {
        clearInterval(timer);
        board = []; current = 1; lastPos = null; errors = 0; totalAttempts = 0; active = false;
    ...
    // NOVA FUNÇÃO: Verifica se ainda existem jogadas válidas
    ...
    function clickCell(r, c, cell) {
        ...
        if(validMove(r, c)) {
            ...
            board[r][c] = current; cell.textContent = current;
            cell.classList.add("valid"); cell.classList.add("current");
            lastPos = [r, c]; current++; updateUI();
            
            // Verifica Vitória ou Fim de Movimentos
            if(current > size * size) {
                finish(true, "completed");
            } else if (!hasAvailableMoves()) {
                finish(false, "no_moves");
            } else {
                highlightPossibleMoves();
            }
        } else {
            ...
            updateUI();
            if(errors >= errorLimits[size]) finish(false, "errors");
        }
    }
    ...
    function finish(win, reason) {
        ...
        modalBox.className = "modal-box " + (win ? "modal-win" : "modal-lose");
        modalIcon.textContent = win ? "🏆" : "💀";
        modalTitle.textContent = win ? "VITÓRIA!" : "DERROTA!";
        modalScoreValue.textContent = score;

        if (reason === "completed") modalReason.textContent = "Excelente! Completou o tabuleiro todo.";
        else if (reason === "no_moves") modalReason.textContent = "Ficou bloqueado sem movimentos possíveis!";
        else if (reason === "timeout") modalReason.textContent = "O seu tempo esgotou-se!";
        else if (reason === "errors") modalReason.textContent = "Atingiu o limite máximo de erros permitidos!";
    ...
    function closeModalAndRestart() {
        ...
        startGame(); // Já recomeça um jogo novo direto
    }

    function saveRank(win, score) {
        let rank = JSON.parse(localStorage.getItem("eliteRank") || "[]");
        rank.push({ player: playerName.value || "Anónimo", size: size, score: score, win: win, timeMode: timeMode });
    ...
    function selectTab(tabSize) {
        ...
        if (filtered.length === 0) {
            rankingDiv.innerHTML = "<p style='color: var(--text-muted); font-size: 14px;'>Nenhuma partida registada.</p>";
            return;
        }
